...
class ExportController extends Controller
{
    public function excel(Request $request)
    {
        ...
        $settledSheet = $spreadsheet->createSheet();
        $settledSheet->setTitle('Settled Reports');
        $cancelledSheet = $spreadsheet->createSheet();
        $cancelledSheet->setTitle('Cancelled Reports');
        $pendingSheet = $spreadsheet->createSheet();
        $pendingSheet->setTitle('Pending Reports');

        // Set headers for all sheets
        foreach ([$settledSheet, $cancelledSheet, $pendingSheet] as $sheet) {
        ...
        // Populate data into corresponding sheets
        foreach ($reports as $report) {
            $sheet = match ($report->status) {
    ...
    public function pdf(Request $request)
    {
        ...
        foreach (['settled', 'cancelled', 'pending'] as $status) {
            $statusReports = $reports->where('status', $status);
            if ($statusReports->isEmpty()) {
                continue;
            }

            $html .= '<h2>' . ucfirst($status) . ' Reports</h2>';
    ...
    public function word(Request $request)
    {
        ...
        foreach (['settled', 'cancelled', 'pending'] as $status) {
            ...
            $section->addTitle(ucfirst($status) . ' Reports', 2);
