<x-app-layout>
    ...
    <a href="{{ route('admin.statisticsCreate') }}" class="text-blue-400 hover:text-blue-600 underline dark:text-blue-300 dark:hover:text-blue-400">
        Create
    </a>
    </div>
    </x-slot> --}}
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="flex items-center justify-between px-4 py-4 bg-white border-b dark:bg-gray-800 dark:border-gray-700 sm:px-6">
                    ...
                    <a href="{{ route('admin.statistics') }}" class="text-blue-400 hover:text-blue-600 underline dark:text-blue-300 dark:hover:text-blue-400">
                        Back
                    </a>
                </div>

                <form action="{{ route('admin.statisticsCreate') }}" method="POST">
                    @csrf
                    @method('POST')
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <tbody>
                            <tr>
                                <th scope="col" class="px-6 py-3">Statistics</th>

                                <td class="px-6 py-4">
                                    <input type="text" name="statistical_graph" id="title" placeholder="Title" class="bg-dark-100 w-full p-4 text-black rounded-lg @error('title') border-0 @enderror" value="{{ old('title') }}">
                                </td>
                            </tr>
                            <!--    <tr>
